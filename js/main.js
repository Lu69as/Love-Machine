...
function Popup(url, id) {
    popupWindow = window.open(
        url, 'popUpWindow' + idx, 'height=400,width=500,left=L,top=T,resizable=no,scrollbars=no,toolbar=no,menubar=no,location=no,directories=no,status=yes');
    idx += 1;
}
...
function love_generation() {
    while (idx <= 5) {
        Popup("https://www.tutorialspoint.com/basics_of_computer_science/images/computer_virus.jpg");
        L + 100;
        L + 100;
        // Window.close.close();
    }
